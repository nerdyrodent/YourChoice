...
Just copy and paste the [Dyadic Framework](DyadicFramework-v2.6.03-Base.txt) into your LLM of choice, or use as a System Prompt in applications like OpenWebUI, text-generation-webui, etc.

It is s LARGE document and so works best with LLMs which have a LARGE context window (such as Nvidias Nemotron, 1M+ context). 8B models (and things with less than 64K context) may struggle a little (read: lots). 14B would be the suggested minimum in 2025. Gemma 3 27B is also OK. Larger models (Grok, Qwen-Max, Claude, DeepSeek) tend to be better.
